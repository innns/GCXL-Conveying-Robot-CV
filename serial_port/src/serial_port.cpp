...
int SerialPort::paramSet(int m_speed, int m_flow_ctrl, int m_databits, int m_stopbits, int m_parity)
{
    ...
    err = e_true_t;
    return err; //edit by innns
}

/*读取满有效字节再返回方式
//data_len为期望收到的长度实际收到的长度为ret的数值
int SerialPort::Recv(unsigned char *rcv_buf,int data_len)
{
    int read_len = 0;
    while(read_len != data_len){
        int real_len;
        real_len = read( fd, &rcv_buf[read_len] , data_len - read_len);   //read函数返回值为实际读到的数据位数
        if(real_len <= 0){
            is_open_=false;
            return -1;   //串口断线，后期需要加入重连
        }
        read_len = read_len + real_len;
    }
    return data_len;
}
*/
